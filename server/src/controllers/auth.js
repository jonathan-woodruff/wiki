...
exports.login = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).cookie('token', token, { httpOnly: true, secure: true, sameSite: true }).json({ //send the user a cookie
            avatar: user.photo
        })
    } catch(error) {
        res.status(500).json({
...
exports.logout = async (req, res) => {
    try {
        return res.status(200).clearCookie('token', { httpOnly: true, secure: true, sameSite: true }).json({
            success: true,
